...
export async function GET(request: NextRequest) {
  ...
  const results = await Promise.all([
    (async () => {
      ...
      const result = await sql(`select data from editions where key @@@ paradedb.match('data.title', $1, conjunction_mode => true)${orderBy} limit 12 offset ${offset};`, [query.toLowerCase()])
